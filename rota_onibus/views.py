...
def rota_onibus(request):
    ...
    paths = [{"path": "rota_onibus/pontos/route_1.geojson", "color": "blue"},
             {"path": "rota_onibus/pontos/route_2.geojson", "color": "red"}]

    for path_route in paths:
        folium.GeoJson(path_route["path"], style_function=lambda feature, color=path_route["color"]: {'color': color}).add_to(m)
    ...
    for local in [{"location": [-10.43029, -45.174006], "popup": "IFPI - Campus Corrente"},
        ...
        icon_bus_stop = CustomIcon(icon_image='bus.png', icon_size=(30, 30))
